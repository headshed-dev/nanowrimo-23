- {{renderer :wordcount_}}
	...
	- I had naievly thought on first glance that docker is being used to orchestrate things but I'm thinking that it isnt, if `docker images` or `docker ps` is anything to go by, however that doesnt mean it doesnt use